...
const BLOCK_META: Record<string, { icon: string; label: string }> = {
  ...
  IMAGE: { icon: '🖼️', label: 'Image + Text' }, VIDEO: { icon: '▶️', label: 'Video' }, BENEFITS: { icon: '✅', label: 'Benefits' },
...
// ── Stable sub-components defined at module level — never recreated on onChange ──────────
// This is the fix for cursor/focus loss. Previously F, Tog, ListField were defined as
// inline consts inside BlockContentEditor, causing React to unmount/remount on every keystroke.
...
function Tog({ label, field, content, onChange }: { label: string; field: string; content: any; onChange: (c: any) => void }) {
  return (
    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', marginBottom: '6px' }}>
      <input type="checkbox" checked={!!content[field]}
        onChange={e => onChange({ ...content, [field]: e.target.checked })}
...
function ListField({ field, placeholder, content, onChange }: { field: string; placeholder: string; content: any; onChange: (c: any) => void }) {
  const items: string[] = content[field] ?? []
  const set = (v: any) => onChange({ ...content, [field]: v })
  return (
    <div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '6px' }}>
        {items.map((item, i) => (
          <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            ...
            <input value={item}
              onChange={e => { const a = [...items]; a[i] = e.target.value; set(a) }}
              placeholder={placeholder} style={{ ...inp, flex: 1 }} />
            <button onClick={() => set(items.filter((_, idx) => idx !== i))}
              style={{ width: '28px', height: '34px', border: '1px solid #fecaca', borderRadius: '6px', background: 'white', color: '#ef4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Trash2 size={10} />
            </button>
          </div>
        ))}
      ...
      <button onClick={() => set([...items, ''])}
        style={{ fontSize: '11px', color: '#7B2FBE', background: 'none', border: '1px dashed #d1d5db', borderRadius: '5px', padding: '4px 9px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px' }}>
        <Plus size={10} /> Add
      </button>
    </div>
  )
...
// ── Block content editor — uses stable F/Tog/ListField components above ──────────────────
  ...
  switch (type) {
    case 'HERO': return (
      <>
          ...
          <F label="Image position">
            <div style={{ display: 'flex', gap: '6px' }}>
              {(['right', 'left'] as const).map(pos => (
                <button key={pos} onClick={() => set('heroImagePosition', pos)}
                  style={{ padding: '5px 12px', borderRadius: '6px', border: (content.heroImagePosition ?? 'right') === pos ? '2px solid #7B2FBE' : '1px solid #e5e7eb', background: (content.heroImagePosition ?? 'right') === pos ? 'rgba(123,47,190,0.06)' : 'white', color: (content.heroImagePosition ?? 'right') === pos ? '#7B2FBE' : '#374151', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        <F label="CTA Button Label"><input value={content.ctaLabel ?? ''} onChange={e => set('ctaLabel', e.target.value)} placeholder="Enrol Now" style={inp} /></F>
        <F label="Below-button text"><input value={content.ctaSubtext ?? ''} onChange={e => set('ctaSubtext', e.target.value)} style={inp} /></F>
        <F label="Badge labels"><ListField field="badgeLabels" placeholder="e.g. Lifetime access" content={content} onChange={onChange} /></F>
      </>
    )
    case 'TEXT': return (
      <>
        <F label="Heading (optional)"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value || undefined)} placeholder="(no heading)" style={inp} /></F>
        <F label="Body"><textarea value={content.body ?? ''} onChange={e => set('body', e.target.value)} rows={5} style={{ ...inp, resize: 'vertical' }} /></F>
        <F label="Alignment">
          <div style={{ display: 'flex', gap: '6px' }}>
            {(['left', 'center'] as const).map(a => (
              <button key={a} onClick={() => set('align', a)}
                style={{ padding: '5px 12px', borderRadius: '6px', border: content.align === a ? '2px solid #7B2FBE' : '1px solid #e5e7eb', background: content.align === a ? 'rgba(123,47,190,0.06)' : 'white', color: content.align === a ? '#7B2FBE' : '#374151', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
    ...
    case 'BENEFITS': return (
      <>
        <F label="Section Heading"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value)} style={inp} /></F>
        <F label="Items"><ListField field="items" placeholder="e.g. Understand the core principles" content={content} onChange={onChange} /></F>
      </>
    )
    case 'CURRICULUM': return (
      <>
        <F label="Section Heading" hint='e.g. "Curriculum", "Programme", "Course Content"'><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value)} style={inp} /></F>
        <Tog label="Show lesson count per module" field="showLessonCount" content={content} onChange={onChange} />
        <Tog label="Show durations (only when data exists)" field="showDurations" content={content} onChange={onChange} />
        <Tog label='Show "Free preview" label' field="showFreePreview" content={content} onChange={onChange} />
        <Tog label="Expand first module by default" field="expandFirst" content={content} onChange={onChange} />
      </>
    )
    case 'INSTRUCTOR': return (
      <>
        <F label="Section Heading"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value)} placeholder="Your Instructor" style={inp} /></F>
        <Tog label="Show instructor avatar" field="showAvatar" content={content} onChange={onChange} />
        <Tog label="Show instructor bio" field="showBio" content={content} onChange={onChange} />
        <F label="Bio override" hint="Leave blank to use the instructor profile bio">
          <textarea value={content.bioOverride ?? ''} onChange={e => set('bioOverride', e.target.value || null)} rows={3} placeholder="(uses instructor profile bio)" style={{ ...inp, resize: 'vertical' }} />
        </F>
      </>
    )
    case 'TESTIMONIALS': return (
      <>
        <F label="Section Heading"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value)} style={inp} /></F>
        <Tog label="Auto-populate from approved reviews" field="pullFromApproved" content={content} onChange={onChange} />
      </>
    )
    case 'FAQ': return (
      <>
        <F label="Section Heading"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value)} style={inp} /></F>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', marginBottom: '7px' }}>
          {(content.items ?? []).map((item: any, i: number) => (
            <div key={i} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '9px' }}>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '5px' }}>
                <input value={item.question}
                  onChange={e => { const a = [...(content.items ?? [])]; a[i] = { ...item, question: e.target.value }; set('items', a) }}
                  placeholder="Question" style={{ ...inp, flex: 1, fontSize: '12px' }} />
                <button onClick={() => set('items', (content.items ?? []).filter((_: any, idx: number) => idx !== i))}
                  style={{ width: '28px', height: '34px', border: '1px solid #fecaca', borderRadius: '6px', background: 'white', color: '#ef4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Trash2 size={10} />
                </button>
              </div>
              <textarea value={item.answer}
                onChange={e => { const a = [...(content.items ?? [])]; a[i] = { ...item, answer: e.target.value }; set('items', a) }}
                rows={2} style={{ ...inp, resize: 'vertical', fontSize: '12px' }} placeholder="Answer" />
            </div>
          ))}
        </div>
        <button onClick={() => set('items', [...(content.items ?? []), { question: '', answer: '' }])}
          style={{ fontSize: '11px', color: '#7B2FBE', background: 'none', border: '1px dashed #d1d5db', borderRadius: '5px', padding: '4px 9px', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px' }}>
          <Plus size={10} /> Add Q
        </button>
      </>
    ...
    case 'CTA': return (
      <>
        <F label="Heading (optional)"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value || undefined)} style={inp} /></F>
        <F label="Button Label"><input value={content.buttonLabel ?? ''} onChange={e => set('buttonLabel', e.target.value)} placeholder="Enrol Now" style={inp} /></F>
        <F label="Below-button text"><input value={content.buttonSubtext ?? ''} onChange={e => set('buttonSubtext', e.target.value)} style={inp} /></F>
        <Tog label="Show product price on button" field="showPrice" content={content} onChange={onChange} />
      </>
    )
    case 'IMAGE': return (
      <>
        ...
        <F label="Layout">
          <div style={{ display: 'flex', gap: '6px' }}>
            {(['image-left', 'image-right', 'full-width'] as const).map(a => (
              <button key={a} onClick={() => set('layout', a)}
                style={{ padding: '5px 12px', borderRadius: '6px', border: (content.layout === a || (!content.layout && a === 'full-width')) ? '2px solid #7B2FBE' : '1px solid #e5e7eb', background: (content.layout === a || (!content.layout && a === 'full-width')) ? 'rgba(123,47,190,0.06)' : 'white', color: (content.layout === a || (!content.layout && a === 'full-width')) ? '#7B2FBE' : '#374151', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                {a === 'image-left' ? '🖼 Left' : a === 'image-right' ? 'Right 🖼' : 'Full width'}
              </button>
            ))}
          </div>
        </F>
        {(content.layout === 'image-left' || content.layout === 'image-right') && (
          <F label="Text beside image" hint="Shown next to the image">
            <textarea value={content.text ?? ''} onChange={e => set('text', e.target.value)} rows={4} style={{ ...inp, resize: 'vertical' }} />
          </F>
        )}
      </>
    )
    case 'VIDEO': return (
      <>
        <F label="YouTube or embed URL" hint="Paste a YouTube link or any iframe embed URL">
          <input value={content.url ?? ''} onChange={e => set('url', e.target.value)} placeholder="https://www.youtube.com/watch?v=..." style={inp} />
        </F>
        <F label="Caption (optional)"><input value={content.caption ?? ''} onChange={e => set('caption', e.target.value || undefined)} style={inp} /></F>
        <F label="Section heading (optional)"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value || undefined)} style={inp} /></F>
      </>
    )
...
function BlockRow({ block, onToggle, onDelete, onMove, isFirst, isLast, expanded, onExpand, onUpdateContent, onDragStart, onDragOver, onDragEnd }: any) {
  ...
  return (
    <div draggable onDragStart={onDragStart} onDragOver={onDragOver} onDragEnd={onDragEnd}
      style={{ background: 'white', border: `1px solid ${expanded ? 'rgba(123,47,190,0.4)' : '#e5e7eb'}`, borderRadius: '10px', overflow: 'hidden', opacity: block.visible ? 1 : 0.55, cursor: 'grab' }}>
      <div onClick={onExpand} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 13px', cursor: 'pointer', background: expanded ? 'rgba(123,47,190,0.03)' : 'white', userSelect: 'none' }}>
        ...
        <button onClick={e => { e.stopPropagation(); onToggle() }}
          style={{ width: '26px', height: '26px', border: '1px solid #e5e7eb', borderRadius: '5px', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: block.visible ? '#7B2FBE' : '#d1d5db', flexShrink: 0 }}>
          {block.visible ? <Eye size={11} /> : <EyeOff size={11} />}
        </button>
        {!singleton && (
          <button onClick={e => { e.stopPropagation(); onDelete() }}
            style={{ width: '26px', height: '26px', border: '1px solid #fecaca', borderRadius: '5px', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ef4444', flexShrink: 0 }}>
            <Trash2 size={10} />
          </button>
        )}
...
export default function SalesPageEditor({ product, allModules = [] }: { product: any; allModules?: any[] }) {
  ...
  const addBlock = (type: string) => {
    ...
    const defaults: Record<string, any> = {
      TEXT: { body: '', align: 'left' }, IMAGE: { src: '', layout: 'full-width', altText: '' },
      VIDEO: { url: '' }, BENEFITS: { heading: 'Benefits', items: [] },
      TESTIMONIALS: { heading: 'Student Reviews', pullFromApproved: true, items: [] },
      FAQ: { heading: 'FAQ', items: [] }, CTA: { buttonLabel: 'Enrol Now', buttonSubtext: '' },
      DIVIDER: { symbol: '✦' },
    }
      ...
      <div>
        <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden', marginBottom: '14px' }}>
          ...
          {promptsOpen && (
            <div style={{ padding: '16px' }}>
              {promptFields.map(f => (
                <PromptField key={f.aKey} question={promptData[f.qKey]} questionKey={f.qKey}
                  answer={promptData[f.aKey]} answerKey={f.aKey} onChange={setP}
                  multiline={f.multi} placeholder={f.placeholder} />
              ))}
            </div>
          )}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', alignItems: 'center' }}>
            <a href={`/courses/${product.slug}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '12px', color: '#6b7280', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '3px' }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', marginBottom: '10px' }}>
          {blocks.length === 0 ? (
            ...
            <BlockRow key={block.id} block={block} isFirst={idx === 0} isLast={idx === blocks.length - 1}
              ...
              onMove={(dir: -1 | 1) => moveBlock(block.id, dir)}
              onUpdateContent={(c: any) => updateBlock(block.id, c)}
        ...
        <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '9px', padding: '12px 14px', marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
            {['TEXT', 'IMAGE', 'VIDEO', 'BENEFITS', 'TESTIMONIALS', 'FAQ', 'CTA', 'DIVIDER'].map(type => {
              const m = BLOCK_META[type]
              return (
                <button key={type} onClick={() => addBlock(type)}
                  style={{ display: 'flex', alignItems: 'center', gap: '3px', padding: '5px 10px', background: 'white', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '11px', fontWeight: 600, color: '#374151', cursor: 'pointer', fontFamily: 'inherit' }}>
                  {m?.icon} {m?.label}
                </button>
              )
            })}
