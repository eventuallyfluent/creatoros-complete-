...
function BlockContentEditor({ type, content, onChange }: { type: string; content: any; onChange: (c: any) => void }) {
  ...
  switch (type) {
    ...
    case 'CTA': return <><F label="Heading (optional)"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value || undefined)} style={inp} /></F><F label="Button Label"><input value={content.buttonLabel ?? ''} onChange={e => set('buttonLabel', e.target.value)} placeholder="Enrol Now" style={inp} /></F><F label="Below-button text"><input value={content.buttonSubtext ?? ''} onChange={e => set('buttonSubtext', e.target.value)} style={inp} /></F><Tog label="Show product price on button" field="showPrice" /></>
    case 'IMAGE': return <><F label="Image URL" hint="Paste a hosted image URL"><input value={content.src ?? ''} onChange={e => set('src', e.target.value)} placeholder="https://..." style={inp} /></F><F label="Alt text"><input value={content.altText ?? ''} onChange={e => set('altText', e.target.value)} placeholder="Describe the image" style={inp} /></F><F label="Caption (optional)"><input value={content.caption ?? ''} onChange={e => set('caption', e.target.value || undefined)} style={inp} /></F><F label="Layout"><div style={{ display:'flex', gap:'6px' }}>{(['image-left','image-right','full-width'] as const).map(a => <button key={a} onClick={() => set('layout', a)} style={{ padding:'5px 12px', borderRadius:'6px', border: content.layout===a||(!content.layout&&a==='full-width') ? '2px solid #7B2FBE' : '1px solid #e5e7eb', background: content.layout===a||(!content.layout&&a==='full-width') ? 'rgba(123,47,190,0.06)' : 'white', color: content.layout===a||(!content.layout&&a==='full-width') ? '#7B2FBE' : '#374151', fontSize:'12px', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>{a==='image-left'?'🖼 Left':'image-right'===a?'Right 🖼':'Full width'}</button>)}</div></F>{(content.layout==='image-left'||content.layout==='image-right') && <F label="Text beside image" hint="Shown next to the image"><textarea value={content.text ?? ''} onChange={e => set('text', e.target.value)} rows={4} style={{ ...inp, resize:'vertical' }} /></F>}</>
    case 'VIDEO': return <><F label="YouTube or embed URL" hint="Paste a YouTube link (e.g. https://www.youtube.com/watch?v=...) or any iframe embed URL"><input value={content.url ?? ''} onChange={e => set('url', e.target.value)} placeholder="https://www.youtube.com/watch?v=..." style={inp} /></F><F label="Caption (optional)"><input value={content.caption ?? ''} onChange={e => set('caption', e.target.value || undefined)} style={inp} /></F><F label="Section heading (optional)"><input value={content.heading ?? ''} onChange={e => set('heading', e.target.value || undefined)} style={inp} /></F></>
...
export default function SalesPageEditor({ product, allModules = [] }: { product: any; allModules?: any[] }) {
  ...
  const addBlock = (type: string) => {
    ...
    const defaults: Record<string,any> = { TEXT: { body:'', align:'left' }, IMAGE: { src:'', layout:'full-width', altText:'' }, VIDEO: { url:'' }, BENEFITS: { heading:'Benefits', items:[] }, TESTIMONIALS: { heading:'Student Reviews', pullFromApproved:true, items:[] }, FAQ: { heading:'FAQ', items:[] }, CTA: { buttonLabel:'Enrol Now', buttonSubtext:'' }, DIVIDER: { symbol:'✦' } }
