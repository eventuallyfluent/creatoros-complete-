...
export default function SalesPageEditor({ course }: { course: any }) {
  const router   = useRouter()
  const prompts  = course.salesPrompts ?? {}
  ...
  const [blocks,       setBlocks]       = useState<any[]>(course.salesPage?.blocks ?? [])
  ...
  const savePrompts = async () => {
    ...
    const res  = await fetch(`/api/courses/${course.id}/sales-page/prompts`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(promptData) })
  ...
  const saveBlocks = async () => {
    ...
    const res  = await fetch(`/api/courses/${course.id}/sales-page/blocks`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ blocks }) })
    const data = await res.json()
    setSavingB(false)
    if (!res.ok) { setError(data.error ?? 'Save failed'); return }
  ...
  const addBlock = (type: string) => {
    ...
    setBlocks(bs => [...bs, { id, type, visible:true, content: defaults[type]??{}, salesPageId: course.salesPage?.id }])
    setExpandedId(id)
  }
      ...
      <div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          ...
          <div style={{ marginLeft:'auto', display:'flex', gap:'8px', alignItems:'center' }}>
            <a href={`/courses/${course.slug}`} target="_blank" rel="noopener noreferrer" style={{ fontSize:'12px', color:'#6b7280', textDecoration:'none', display:'flex', alignItems:'center', gap:'3px' }}><ExternalLink size={11}/> Preview</a>
